...
namespace KenseiECS.Editor
{
    ...
    {
        Vector2 _scrollPos;
        Vector2 _stackScrollPos;
        string _searchFilter = "";
        int _selectedEvent = -1;
        ProfileEventType? _typeFilter;

        // Entity navigation — back/forward history
        int? _focusedEntity;
        readonly List<int> _navHistory = new();
        int _navIndex = -1;

        IReadOnlyList<ProfileEvent> _events;
        ...
        void DrawFilterButton(string label, ProfileEventType? type)
        {
            var oldColor = GUI.backgroundColor;
            if (_typeFilter == type) GUI.backgroundColor = Color.cyan;
        ...
        void DrawEventRow(ProfileEvent evt, int index)
        {
        ...
        Color GetEventColor(ProfileEventType type)
        {
            return type switch
            {
        ...
        void NavigateTo(int entityIndex)
        {
        ...
        bool CanGoBack() => _navIndex > 0;
        bool CanGoForward() => _navIndex < _navHistory.Count - 1;
